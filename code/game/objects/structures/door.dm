//NOT mineral_doors
//basically just a straight rip from thief manor - essentially the same as mineral_doors but w/e overlay iirc
...
/obj/structure/door
	...
	/// Last time the door was knocked on or rattled
	var/last_knock_rattle_time = 0
...
/obj/structure/door/attack_hand(mob/user)
	...
	if(isliving(user))
		...
		if(L.m_intent == MOVE_INTENT_SNEAK)
			...
			return
	if(can_knock)
		if(world.time < last_knock_rattle_time + 1 SECONDS)
			return
		last_knock_rattle_time = world.time
...
/obj/structure/door/bulkhead/rattle(silent = FALSE)
	return

/obj/structure/door/grate
	name = "metal grate"
	desc = "A metal grate. You can see through the gaps even when closed."
	icon_state = "grate"
	armor = list("blunt" = 10, "slash" = 25, "stab" = 25, "piercing" = 0, "fire" = 40, "acid" = 40)
	max_integrity = 1500
	...
	break_sound = 'sound/combat/hits/onmetal/grille (2).ogg'
	repair_cost_first = /obj/item/ingot/iron
	repair_cost_second = /obj/item/ingot/iron
	repair_skill = /datum/skill/craft/blacksmithing
	animate_time = 0.6 SECONDS

/obj/structure/door/grate/Close(silent = FALSE)
	if(switching_states || !door_opened)
		return
	for(var/mob/living/L in get_turf(src))
		return
	...
	if(!silent)
		playsound(get_turf(src), close_sound, 90)
	// Always keep opacity FALSE for grate doors, even when closed
	set_opacity(FALSE)
	flick("[initial(icon_state)]closing", src)
	sleep(animate_time)
	density = TRUE
...
/obj/structure/door/grate/force_closed()
	switching_states = TRUE
	// Always keep opacity FALSE for grate doors, even when closed
	set_opacity(FALSE)
	density = TRUE
